...
function switchAuthTab(mode) { isLoginMode = mode === 'login'; loginTab.classList.toggle('selected', isLoginMode); registerTab.classList.toggle('selected', !isLoginMode); updateUIText(); }
...
function openMapScreen() { showScreen('map'); const map2 = document.getElementById('map2'); const map2LockedText = document.getElementById('map2Locked'); if (playerData.megalodonDefeatedOn === 'normal' || playerData.megalodonDefeatedOn === 'jaws') { map2.classList.remove('locked'); map2LockedText.style.display = 'none'; } else { map2.classList.add('locked'); map2LockedText.style.display = 'block'; } }